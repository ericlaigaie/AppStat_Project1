...
par(mfrow=c(2,2))
plot(model2_step)
ggplot(train1, aes(x=model2_step$residuals)) + geom_histogram(fill='gray', color='black') + stat_function(fun = dnorm, args = list(mean = mean(model2_step$residuals), sd = sd(model2_step$residuals))) + labs(title='Residual Histogram', x='Residuals', y='Count')
...
# Test MSE for Lasso
testMSE = mean((exp(test_response) - exp(test_predicted))^2)
testMSE # .2149
...
# RMSE -- .0489
mean((exp(test_response) - exp(test_predictions))^2)
...
# LASSO:  .2188  /    711,600,945
# KNN:    .0799  /    109,058,785

# Validation MSE
# SIMPLE: N/A    /            N/A
# LASSO:  .2068  /    785,269,607
# KNN:    .0396  /    133,047,991
